...
$action = $isEdit ? "/admin/brands/{$formData['id']}" : '/admin/brands';
    ...
    <div class="container-fluid">
        <nav aria-label="breadcrumb">
            <ol class="breadcrumb mb-2">
                ...
                <li class="breadcrumb-item"><a href="/admin/brands" class="text-muted text-decoration-none">Бренди</a></li>
        ...
        <div class="d-flex align-items-start justify-content-between mb-4">
            <div>
                <h1 class="display-6 mb-0"><?= $isEdit ? 'Редагування бренду' : 'Новий бренд' ?></h1>
                <p class="text-muted small mb-0"><?= $isEdit ? 'Оновіть інформацію про бренд' : 'Заповніть форму для створення нового бренду' ?></p>
            </div>
            <a href="/admin/brands" class="btn btn-outline-secondary btn-sm d-inline-flex align-items-center gap-2"><i class="bi bi-arrow-left"></i> Назад</a>
        </div>
            ...
            <form action="<?= $action ?>" method="POST">
                ...
                <div class="row g-4 mb-4">
                    <div class="col-md-6">
                        ...
                        <input type="text" id="name" name="name" class="form-control" value="<?= htmlspecialchars($formData['name'] ?? '', ENT_QUOTES, 'UTF-8') ?>" placeholder="Наприклад: BMW" required>
                    </div>

                    <div class="col-md-6">
                        <label for="slug" class="form-label">Slug (ЛП URL)</label>
                        <input type="text" id="slug" name="slug" class="form-control" value="<?= htmlspecialchars($formData['slug'] ?? '', ENT_QUOTES, 'UTF-8') ?>" placeholder="napr-bmw" required>
                    </div>
                </div>
                ...
                <div class="d-flex gap-2">
                    ...
                    <a href="/admin/brands" class="btn btn-outline-secondary">Скасувати</a>
                </div>
            </form>
        </div>
    </div>
