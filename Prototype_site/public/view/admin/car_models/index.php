...
if (!empty($search)) {
    foreach ($items as $item) {
        if (stripos($item['model'] ?? '', $search) !== false || stripos($item['brand'] ?? '', $search) !== false || stripos((string)($item['id'] ?? ''), $search) !== false) {
            $filtered[] = $item;
        }
    }
} else {
    ...
    <div class="container-fluid">
        <nav aria-label="breadcrumb">
            <ol class="breadcrumb mb-2">
                ...
                <li class="breadcrumb-item active" aria-current="page">Моделі авто</li>
            </ol>
        </nav>

        <div class="d-flex align-items-start justify-content-between mb-4">
            <div>
                <h1 class="display-6 mb-0">Моделі авто</h1>
                <p class="text-muted small mb-0">Керування автомобільними моделями</p>
            </div>
            <div class="d-flex gap-2">
                <a href="/admin/car-models" class="btn btn-outline-secondary btn-sm d-inline-flex align-items-center gap-2"><i class="bi bi-arrow-clockwise"></i> Оновити</a>
                <a href="/admin/car-models/create" class="btn btn-dark btn-sm d-inline-flex align-items-center gap-2"><i class="bi bi-plus-lg"></i> Нова модель</a>
            </div>
        </div>
        ...
        <div class="row g-3 mb-4">
            <div class="col-lg-4 col-md-6">
                <div class="admin-card filters-card p-4">
                    ...
                    <form id="modelsToolbar" class="row g-3">
                        <div class="col-12">
                            <div class="input-group">
                                <span class="input-group-text bg-white border-end-0"><i class="bi bi-search"></i></span>
                                <input id="modelsSearch" name="q" class="form-control" type="search" placeholder="Модель, бренд, ID" value="<?= htmlspecialchars($search, ENT_QUOTES, 'UTF-8') ?>">
                            </div>
                        </div>
                        <div class="col-6">
                        ...
                        <div class="col-6 text-end">
                            <a href="/admin/car-models" class="btn btn-outline-secondary btn-sm w-100">Очистити</a>
                        </div>
                    </form>
                </div>
            </div>
                ...
                <div class="row g-3">
                    <div class="col-md-6">
                        <div class="admin-card p-4">
                            <div class="small text-muted">Всього моделей</div>
        ...
        <div class="admin-card p-4">
            <div class="d-flex justify-content-between align-items-center mb-3">
                <h5 class="mb-0">Список моделей</h5>
                <div class="small text-muted">Показано <?= count($paginated) ?> записів</div>
            </div>

            <?php if (empty($paginated)): ?>
                <div class="alert alert-info mb-0">Моделей не знайдено.</div>
            <?php else: ?>
                <div class="table-responsive">
                    <table class="table table-admin align-middle mb-0">
                        ...
                        <tbody>
                            <?php foreach ($paginated as $m): ?>
                                <tr>
                                    <td class="fw-semibold"><?= (int)($m['id'] ?? 0) ?></td>
                                    <td><?= htmlspecialchars($m['brand'] ?? '', ENT_QUOTES, 'UTF-8') ?></td>
                                    <td class="fw-semibold"><?= htmlspecialchars($m['model'] ?? '', ENT_QUOTES, 'UTF-8') ?></td>
                                    <td class="small"><?= htmlspecialchars($m['generation'] ?? '—', ENT_QUOTES, 'UTF-8') ?></td>
                                    <td class="small text-muted"><?= htmlspecialchars(($m['year_from'] ?? '?') . ' - ' . ($m['year_to'] ?? '?'), ENT_QUOTES, 'UTF-8') ?></td>
                                    <td class="text-end">
                                        <form action="/admin/car-models/<?= (int)($m['id'] ?? 0) ?>/delete" method="POST" class="d-inline-block">
