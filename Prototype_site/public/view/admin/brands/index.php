...
<section class="py-4">
    <div class="container-fluid">
        ...
        <div class="d-flex align-items-start justify-content-between mb-4">
            <div>
                <h1 class="display-6 mb-0">Бренди</h1>
                <p class="text-muted small mb-0">Керування брендами в каталозі</p>
            </div>
            <div class="d-flex gap-2">
                <a href="/admin/brands" class="btn btn-outline-secondary btn-sm d-inline-flex align-items-center gap-2"><i class="bi bi-arrow-clockwise"></i> Оновити</a>
                <a href="/admin/brands/create" class="btn btn-dark btn-sm d-inline-flex align-items-center gap-2"><i class="bi bi-plus-lg"></i> Новий бренд</a>
            </div>
        </div>
        ...
        <div class="row g-3 mb-4">
            <div class="col-lg-4 col-md-6">
                <div class="admin-card filters-card p-4">
                    ...
                    <form id="brandsToolbar" class="row g-3">
                        <div class="col-12">
                            <div class="input-group">
                                <span class="input-group-text bg-white border-end-0"><i class="bi bi-search"></i></span>
                                <input id="brandsSearch" name="q" class="form-control" type="search" placeholder="Пошук за назвою" value="<?= htmlspecialchars($search, ENT_QUOTES, 'UTF-8') ?>">
                            </div>
                        </div>
                        <div class="col-6">
                        ...
                        <div class="col-6 text-end">
                            <a href="/admin/brands" class="btn btn-outline-secondary btn-sm w-100">Очистити</a>
                        </div>
                    </form>
                </div>
            </div>
                ...
                <div class="row g-3">
                    <div class="col-md-6">
                        <div class="admin-card p-4">
                            <div class="small text-muted">Всього брендів</div>
        ...
        <div class="admin-card p-4">
            ...
            <?php if (empty($paginated)): ?>
                <div class="alert alert-info mb-0">Брендів не знайдено.</div>
            <?php else: ?>
                    ...
                    <table class="table table-admin align-middle mb-0">
                        ...
                        <tbody>
                            <?php foreach ($paginated as $b): ?>
                                <tr>
                                    <td class="fw-semibold"><?= htmlspecialchars($b['name'] ?? '', ENT_QUOTES, 'UTF-8') ?></td>
                                    <td><code class="small"><?= htmlspecialchars($b['slug'] ?? '', ENT_QUOTES, 'UTF-8') ?></code></td>
                                    <td class="text-end">
                                        <a href="/admin/brands/<?= (int)($b['id'] ?? 0) ?>/edit" class="btn btn-sm btn-outline-primary mb-2">Редагувати</a>
                                        <form action="/admin/brands/<?= (int)($b['id'] ?? 0) ?>/delete" method="POST" class="d-inline-block ms-1">
